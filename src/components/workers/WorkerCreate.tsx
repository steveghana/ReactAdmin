...
export const MyToolbar: React.FC<IToolbar> = props => (
    <Toolbar style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        <SaveButton onClick={() => props.setClicked(false)} style={{ width: '420px' }} variant="contained">
            Add new worker
        </SaveButton>
        {!props.noPermission && (
            <Button onClick={() => props.setClicked(prev => !prev)} style={{ width: '420px' }} variant="outlined">
        ...
        <div>
            <Create>
                {!clicked ? (
                    ...
                    <SimpleForm toolbar={<MyToolbar noPermission={true} setClicked={setClicked} />}>
